...
public class MethodCodeGenerateAction extends AnAction {
    ...
    public MethodCodeGenerateAction() {
        getTemplatePresentation().setText("add test method to the testClass");
    }
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (!Files.exists(ValueContext.getPath())) {
            String message = ("the unitTest class is not exists ,please generate test class first");
            Messages.showMessageDialog(project, message, "Generate Failed", null);
            return;
        }
        boolean valueSource = false;
        try {
            valueSource = Files.readAllLines(ValueContext.getPath()).stream().anyMatch(
